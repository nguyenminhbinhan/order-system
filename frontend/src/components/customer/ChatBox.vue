...
const fetchMessages = async () => {
  try {
    const res = await apiClient.get(`/messages/${props.tableId}`);
...
const sendMessage = async () => {
  ...
  try {
    await apiClient.post('/messages', { tableId: props.tableId, sender: 'customer', content });
  } catch(error) {
...
<template>
  <div class="flex flex-col h-80 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl overflow-hidden shadow-lg mt-4 mb-4">
    ...
    <div class="flex-1 overflow-y-auto p-4 space-y-3 bg-slate-50 dark:bg-slate-900/50" ref="messagesContainer">
      <div v-if="messages.length === 0" class="text-center text-slate-500 text-sm mt-4">Không có dữ liệu</div>
    ...
    <div class="p-3 bg-white dark:bg-slate-900 border-t border-slate-200 dark:border-slate-800 flex gap-2">
      ...
      <button @click="sendMessage" :disabled="!newMessage.trim() || isSending" class="bg-primary text-white p-2 w-10 h-10 rounded-xl disabled:opacity-50 transition-colors flex items-center justify-center shrink-0">
        <span v-if="isSending" class="material-symbols-outlined animate-spin text-[18px] leading-none">refresh</span>
        <span v-else class="material-symbols-outlined text-[18px] leading-none">send</span>
      </button>
    </div>
  </div>
</template>
