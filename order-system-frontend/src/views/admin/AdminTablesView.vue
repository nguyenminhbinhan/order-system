...
const openAddModal = () => {
  ...
  formData.value = { id: null, name: `Table ${tables.value.length + 1}`, qrCode: `http://localhost:5173/menu?tableId=${tables.value.length + 1}`, status: 'empty' };
      ...
      <div class="p-8 max-w-[1100px] mx-auto w-full">
        ...
        <div class="flex flex-wrap items-center justify-between gap-4 mb-6">
          <div class="flex gap-2 overflow-x-auto pb-2">
            ...
            <button @click="activeFilter = 'empty'" :class="activeFilter === 'empty' ? 'bg-primary text-white border-primary' : 'bg-white dark:bg-slate-900 text-slate-600 dark:text-slate-300 border-slate-200 dark:border-slate-800'" class="px-5 py-2 rounded-full border text-sm font-semibold whitespace-nowrap transition-colors">Empty</button>
            <button @click="activeFilter = 'waiting_confirmation'" :class="activeFilter === 'waiting_confirmation' ? 'bg-primary text-white border-primary' : 'bg-white dark:bg-slate-900 text-slate-600 dark:text-slate-300 border-slate-200 dark:border-slate-800'" class="px-5 py-2 rounded-full border text-sm font-semibold whitespace-nowrap transition-colors">Waiting Orders</button>
            ...
            <button @click="activeFilter = 'needs_payment'" :class="activeFilter === 'needs_payment' ? 'bg-primary text-white border-primary' : 'bg-white dark:bg-slate-900 text-slate-600 dark:text-slate-300 border-slate-200 dark:border-slate-800'" class="px-5 py-2 rounded-full border text-sm font-semibold whitespace-nowrap transition-colors">Needs Payment</button>
          </div>
            ...
            <table class="w-full text-left border-collapse">
              ...
              <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                <tr v-for="table in filteredTables" :key="table.id" class="hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors group" :class="{ 'opacity-50 grayscale bg-slate-50 dark:bg-slate-800/50': table.isDeleted }">
                  ...
                  <td class="px-6 py-4">
                    <span :class="['inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold uppercase', getStatusBadgeClass(table.status)]">
                ...
                <tr v-if="filteredTables.length === 0">
                  <td colspan="5" class="px-6 py-12 text-center text-slate-500">
                    ...
                    <p>No tables found matching criteria.</p>
                  </td>
                </tr>
              </tbody>
            </table>
      ...
      <div class="bg-white dark:bg-slate-900 rounded-2xl shadow-xl w-full max-w-md overflow-hidden">
        ...
        <form @submit.prevent="submitForm" class="p-6 space-y-4">
          ...
          <div>
            ...
            <input v-model="formData.qrCode" required type="url" class="w-full rounded-lg border-slate-200 dark:border-slate-700 dark:bg-slate-800 focus:border-primary focus:ring-primary shadow-sm text-xs font-mono" />
          </div>
          <div v-if="isEditing">
            <label class="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-1">Current Status</label>
            <select v-model="formData.status" required class="w-full rounded-lg border-slate-200 dark:border-slate-700 dark:bg-slate-800 focus:border-primary focus:ring-primary shadow-sm">
